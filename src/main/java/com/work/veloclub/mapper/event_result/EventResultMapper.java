...
import com.work.veloclub.model.event_result.EventStartProtocolResponse;
...
public class EventResultMapper {
    ...
    public static EventStartProtocolResponse mapToEventStartProtocolResponse(EventResult eventResult){
        String team = null;
        if (eventResult.getUserProfile().getTeam() != null){
            team = eventResult.getUserProfile().getTeam().getName();
        }
        return new EventStartProtocolResponse(
                eventResult.getId(),
                eventResult.getStartOrder(),
                eventResult.getStartTime(),
                new UserShortResponse(
                        eventResult.getUserProfile().getId(),
                        eventResult.getUserProfile().getFirstName(),
                        eventResult.getUserProfile().getLastName(),
    ...
    public static List<EventStartProtocolResponse> mapToEventStartProtocolResponse(List<EventResult> eventResult){
        return eventResult.stream().map(EventResultMapper::mapToEventStartProtocolResponse).toList();
    }
}
